...
run-pgsql-test:
	...
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz sql --workers=3 --auth=examples/pgsql/secret.json --config=examples/pgsql/sql.csv --verbose
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz sql --workers=3 --auth=examples/pgsql/secret.json --config=examples/pgsql/sql.csv --verbose --rps=1
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz sql --workers=3 --auth=examples/pgsql/secret.json --config=examples/pgsql/sql.csv --duration=3s
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz sql --workers=10 --auth=examples/pgsql/secret.yaml --config=examples/pgsql/sql.csv --duration=3s
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz sql --workers=100 --auth=examples/pgsql/secret.yaml --config=examples/pgsql/sql.csv --duration=3s
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz sql --workers=20 --auth=examples/pgsql/secret.yaml --config=examples/pgsql/sql.csv --duration=10s --rps=50 --output=files/$$(date +%Y%m%d_%H%M%S)_result.json
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz sql --workers=200 --auth=examples/pgsql/secret.yaml --config=examples/pgsql/sql.csv --duration=10s --rps=500 --output=files/$$(date +%Y%m%d_%H%M%S)_result.json
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz sql --workers=400 --auth=examples/pgsql/secret.yaml --config=examples/pgsql/sql.csv --duration=10s --rps=1000 --output=files/$$(date +%Y%m%d_%H%M%S)_result.json
...
run-api-test:
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz api --workers=3 --auth=examples/api/secret.json --config=examples/api/api.json --verbose
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz api --workers=3 --auth=examples/api/secret.json --config=examples/api/api.json --verbose --rps=1
	PROMETHEUS_PUSH_GATEWAY=${PROMETHEUS_PUSHGATEWAY_URL} ./etz api --workers=3 --auth=examples/api/secret.json --config=examples/api/api.yaml --duration=3s
